...
public class IcoSphere {
	...
	static Pair<List<Vector3>, List<Face>> subdivide(List<Vector3> vertices, List<Face> faces) {
		...
		for (Face face : faces) {
			Vector3 v1 = vertices.get(face.v0()).copy();
			Vector3 v2 = vertices.get(face.v1()).copy();
			Vector3 v3 = vertices.get(face.v2()).copy();
			v1.normalize();
			v2.normalize();
			v3.normalize();
	...
	public static Pair<List<Vector3>, List<Face>> icosphere(int iterations) {
		...
		for (int i = 0; i < vertices.size(); i++) {
			vertices.set(i, vertices.get(i).copy().normalize());
		}
