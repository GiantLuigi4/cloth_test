...
public class PointBuilder {
	...
	public AbstractPoint build(boolean soft) {
		if (refs.size() == 0) return null;
		if (soft) return new SoftBodyPoint(origin, refs.toArray(EMPTY), origin.copy());
		else return new Point(origin, refs.toArray(EMPTY), EMPTY);
	}
}
